...
def list_tables(
    ...
    try:
        ...
        if format_output == "json":
            ...
            if not tables:
                print(
                    f"No tables found in database '{database_name}'.", file=sys.stderr
                )
                return
...
def main_list_databases():
    ...
    parser = argparse.ArgumentParser(
        description="List databases in your Treasure Data account"
    )
    parser.add_argument(
        "--api-key",
        help="Treasure Data API key (if not provided, uses TD_API_KEY env var)",
    )
    parser.add_argument(
        "--endpoint",
        default="api.treasuredata.com",
        help="API endpoint (default: api.treasuredata.com for US, use co.jp for Japan)",
    )
    parser.add_argument(
        "--format",
        dest="format_output",
...
def main_list_tables():
    ...
    parser.add_argument(
        "database_name", help="Name of the database to retrieve tables from"
    )
    parser.add_argument(
        "--api-key",
        help="Treasure Data API key (if not provided, uses TD_API_KEY env var)",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "list":
            ...
            print(
                "Unknown command. Available commands: list, get, tables",
                file=sys.stderr,
            )
            print("Usage:", file=sys.stderr)
            print("  python -m td_mcp_server.cli_api list [options]", file=sys.stderr)
            print(
                "  python -m td_mcp_server.cli_api get DATABASE_NAME [options]",
                file=sys.stderr,
            )
            print(
                "  python -m td_mcp_server.cli_api tables DATABASE_NAME [options]",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
        print(
            "Command required. Available commands: list, get, tables", file=sys.stderr
        )
        print("Usage:", file=sys.stderr)
        print("  python -m td_mcp_server.cli_api list [options]", file=sys.stderr)
        print(
